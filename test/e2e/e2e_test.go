...
var _ = Describe("Manager", Ordered, func() {
	...
	BeforeAll(func() {
		...
		cmd = exec.Command("make", "deploy-e2e", fmt.Sprintf("IMG=%s", projectImage), fmt.Sprintf("MGMT_HOST=%s", managementIP))
		...
		Context("Ingress", Ordered, func() {
			...
			When("Service is annotated with policy", Ordered, func() {
				...
				It("should add service to policy status", func() {
					Eventually(func(g Gomega) {
						out, err := utils.Run(exec.Command("kubectl", "get", "NBPolicy", "default", "-o", "jsonpath={.status.managedServiceList}"))
						g.Expect(err).NotTo(HaveOccurred())
				...
				It("should create policy on NetBird", func() {
					Eventually(func(g Gomega) {
						out, err := utils.Run(exec.Command("kubectl", "get", "NBPolicy", "default", "-o", "jsonpath={.status.tcpPolicyID}"))
						g.Expect(err).NotTo(HaveOccurred())
					...
					Eventually(func(g Gomega) {
						...
						g.Expect(policies[0].Name).To(Equal("Kubernetes Default Group"))
					})
				})
			})
		})
